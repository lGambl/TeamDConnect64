...
GameWindow::GameWindow(int width, int height, const char *title) :
	...
	if (width < 300) {
		throw std::invalid_argument(
				"Width must be greater than or equal to 300");
	}

	if (height < 300) {
		throw std::invalid_argument(
				"Height must be greater than or equal to 300");
	}
	...
	int closeButtonY = height - heightCentering
			- (closeButtonHeight / (this->numberColumns / 2)) + 5; // FIXME: Doesn't align properly when altering size of grid
	...
	for (int i = 0; i < numberRows; i++) {
		for (int j = 0; j < numberColumns; j++) {
			...
			if (newNode->getNumber() < 1) {
				EmptyNode *newEmptyBox = new EmptyNode(inputX, inputY,
						inputBoxWidth, inputBoxHeight, "", newNode);
				newEmptyBox->textsize(inputBoxFontSize++); // FIXME: Fix font size
				this->gameBoard.push_back(newEmptyBox);
			} else {
				ExistingNode *newBox = new ExistingNode(inputX, inputY,
						inputBoxWidth, inputBoxHeight, newNode);
	...
	this->closeButton = new Fl_Button(closeButtonX, closeButtonY,
